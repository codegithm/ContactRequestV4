...
export default function PartnerEditDialog({
  ...
  const [isMobileAddFieldStepOpen, setIsMobileAddFieldStepOpen] =
    useState(false);
  const [desktopFieldDrawerId, setDesktopFieldDrawerId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    setThemeBadgeStyle(
      partner.theme?.badgeStyle === "solid" ? "solid" : "soft",
    );
    setRadixAccentColor(
      partner.theme?.radix?.accentColor ?? defaultRadixAccentColor,
    );
    ...
    setRadixPanelBackground(
      partner.theme?.radix?.panelBackground ?? defaultRadixPanelBackground,
    );
    setRadixAppearance(
      partner.theme?.radix?.appearance ?? defaultRadixAppearance,
    );
    setVdn(
      typeof partner.settings?.vdn === "string" ? partner.settings.vdn : "",
    );
    setBrokerCode(
      typeof partner.settings?.brokerCode === "string"
    ...
    setReportSearchFieldIds(
      partner.feedbackSearchFields?.map((field) => field.id) ?? [],
    );
    setConsentEnabled(!!partner.consentCheckbox?.enabled);
    setConsentRequired(partner.consentCheckbox?.required !== false);
    setConsentLabel(partner.consentCheckbox?.label || defaultConsentLabel);
    const referralSection = partner.sections?.find(isReferralSection);
    setEnableReferralSection(!!referralSection);
    setReferralAddEntryLabel(
      referralSection?.addEntryLabel || "Add Referred Contact",
    );
    setReferralMaxEntries(
      referralSection?.maxEntries ? String(referralSection.maxEntries) : "",
    );
  ...
  const availableFieldOptions = useMemo(
    () => getAvailableContactRequestFields(fields.map((f) => f.id)),
    [fields],
  );
  const groupedAvailableFieldOptions = useMemo(
    () => getGroupedAvailableContactRequestFields(fields.map((f) => f.id)),
    [fields],
  );
  ...
  const addField = () => {
    ...
    const selectedSection =
      sections.find((s) => s.id === newFieldSectionId) || null;
    if (!selectedSection) {
  ...
  const handleSave = async () => {
    ...
    const fieldsForSave = enableReferralSection
      ? fields
      : fields.filter(
          (f) => !f.sectionId || !referralSectionIds.has(f.sectionId),
        );
    ...
    const updatedConfig = updatePartnerFields(
      {
        ...
        theme: {
          ...
          radix: {
            ...partner.theme?.radix,
            accentColor:
              radixAccentColor as PartnerConfig["theme"]["radix"]["accentColor"],
            grayColor:
              radixGrayColor as PartnerConfig["theme"]["radix"]["grayColor"],
            radius: radixRadius as PartnerConfig["theme"]["radix"]["radius"],
            scaling: radixScaling as PartnerConfig["theme"]["radix"]["scaling"],
            panelBackground:
              radixPanelBackground as PartnerConfig["theme"]["radix"]["panelBackground"],
            appearance:
              radixAppearance as PartnerConfig["theme"]["radix"]["appearance"],
          },
        },
        settings:
        ...
        <details className="rounded-lg border border-border bg-muted/20 p-4">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            <AdminSettingsFields
              ...
              onEnableReportChange={setFeedback}
      ...
      <Drawer open={open} onOpenChange={handleModalOpenChange}>
        <DrawerContent className="max-h-[92vh]">
          ...
          <div className="overflow-y-auto px-4 py-4">
            {isMobileAddFieldStepOpen ? (
              <AddFieldPanel {...addFieldPanelProps} showAddButton={false} />
            ) : (
              formSections
            )}
            {!isMobileAddFieldStepOpen && (
