...
export default function PartnerGrid({ partners, onPartnersChanged, }: PartnerGridProps) {
    ...
    const handleTogglePartner = async (partner: PartnerConfig, nextActive: boolean) => {
    ...
    return (<motion.section className="space-y-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ ...springTransition, delay: 0.2 }}>
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant={statusFilter === "all" ? "secondary" : "outline"} onClick={() => setStatusFilter("all")}>
            All ({partners.length})
          </Button>
          <Button type="button" variant={statusFilter === "active" ? "secondary" : "outline"} onClick={() => setStatusFilter("active")}>
            Active ({activeCount})
          </Button>
          <Button type="button" variant={statusFilter === "inactive" ? "secondary" : "outline"} onClick={() => setStatusFilter("inactive")}>
      ...
      <div className="rounded-lg border border-border bg-card form-container">
        <div className="border-b border-border p-4">
          <div className="relative max-w-sm">
            ...
            <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by partner name, id, status..." className="pl-9"/>
          </div>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            ...
            const totalFieldCount = partner.sections?.length
                ? partner.sections.reduce((total, section) => total + section.fields.length, 0)
                : partner.fields.length;
            return (<TableRow key={partner.partnerId}>
                  <TableCell>
                    <div className="space-y-1">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <Switch checked={isActive} disabled={isSaving} onCheckedChange={(checked) => handleTogglePartner(partner, checked)}/>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {isActive ? (<Link href={`/${partner.partnerId}`} className="text-sm font-medium text-foreground underline underline-offset-4">
                        View form
                      </Link>) : (<span className="text-sm text-muted-foreground">
                        Disabled
                      </span>)}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center justify-end gap-2">
                      <Button type="button" variant="outline" size="sm" onClick={() => setEditingPartner(partner)}>
                        Edit
                      </Button>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleTogglePartner(partner, !isActive)}>
      ...
      <PartnerEditDialog partner={editingPartner} open={!!editingPartner} onOpenChange={(open) => {
            if (!open)
