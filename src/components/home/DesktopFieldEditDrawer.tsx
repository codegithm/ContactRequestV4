...
export default function DesktopFieldEditDrawer({ open, field, sectionOptions, onUpdate, onClose, }: DesktopFieldEditDrawerProps) {
    return (<Drawer open={open} onOpenChange={(next) => {
            if (!next)
                onClose();
        }}>
      <DrawerContent className="max-h-[88vh]">
        ...
        <div className="space-y-4 overflow-y-auto px-4 py-4">
          {field && (<FieldEditorForm field={field} sectionOptions={sectionOptions} onUpdate={(updates) => onUpdate(field.id, updates)}/>)}
        </div>
