...
import { createEditableFieldFromCatalog, createSectionId, getAvailableContactRequestFields, getGroupedAvailableContactRequestFields, type EditablePartnerField, type EditablePartnerSection, updatePartnerFields, } from "@/components/home/partner-management-utils";
import { defaultThemePrimary, defaultThemePrimaryForeground, defaultThemeRadius, defaultThemePageBackground, defaultThemeHeaderBackground, defaultThemeFooterBackground, defaultThemeCardBackground, defaultThemeCardBorder, defaultThemeHeadlineColor, defaultThemeDescriptionColor, defaultThemeBodyTextColor, defaultRadixAccentColor, defaultRadixGrayColor, defaultRadixRadius, defaultRadixScaling, defaultRadixPanelBackground, defaultRadixAppearance, defaultFooterLayout, defaultFooterPoweredByLabel, defaultFooterLinksJson, defaultFooterLogosJson, springTransition, } from "@/components/home/home-page-constants";
import { useVdnDirectory } from "@/hooks/useVdnDirectory";
import { createPartnerConfigStore } from "@/lib/partner-config-store";
import type { PartnerConfig, PartnerFooterLink, PartnerFooterLogo, } from "@/lib/partners";
import { serializeExtraSettingDrafts, type ExtraSettingDraft, } from "@/lib/partner-settings";
...
export default function PartnerAdminPanel({ onSaved }: PartnerAdminPanelProps) {
    ...
    const [footerLayout, setFooterLayout] = useState<"split" | "centered" | "links-only">(defaultFooterLayout);
    const [footerShowPoweredBy, setFooterShowPoweredBy] = useState(true);
    const [footerPoweredByLabel, setFooterPoweredByLabel] = useState(defaultFooterPoweredByLabel);
    const [footerPoweredByUrl, setFooterPoweredByUrl] = useState("");
    const [footerLinksJson, setFooterLinksJson] = useState(defaultFooterLinksJson);
    const [footerLogosJson, setFooterLogosJson] = useState(defaultFooterLogosJson);
    ...
    const availableFieldOptions = useMemo(() => getAvailableContactRequestFields(fields.map((field) => field.id)), [fields]);
    const groupedAvailableFieldOptions = useMemo(() => getGroupedAvailableContactRequestFields(fields.map((field) => field.id)), [fields]);
    const groupedFields = useMemo(() => sections.map((section) => ({
        ...section,
    ...
    const handleReferralToggle = (checked: boolean) => {
        ...
        if (checked) {
            setSections((current) => current.some((section) => section.id === "refer")
                ? current
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        const sectionDefinitions = sections
            ...
            addEntryLabel: section.id === "refer"
        ...
        const config: PartnerConfig = {
            ...
            theme: {
                ...
                primaryForeground: themePrimaryForeground.trim() || defaultThemePrimaryForeground,
                radius: themeRadius.trim() || defaultThemeRadius,
                pageBackground: themePageBackground.trim() || defaultThemePageBackground,
                headerBackground: themeHeaderBackground.trim() || defaultThemeHeaderBackground,
                footerBackground: themeFooterBackground.trim() || defaultThemeFooterBackground,
                cardBackground: themeCardBackground.trim() || defaultThemeCardBackground,
                cardBorder: themeCardBorder.trim() || defaultThemeCardBorder,
                headlineColor: themeHeadlineColor.trim() || defaultThemeHeadlineColor,
                descriptionColor: themeDescriptionColor.trim() || defaultThemeDescriptionColor,
                bodyTextColor: themeBodyTextColor.trim() || defaultThemeBodyTextColor,
                badgeStyle: themeBadgeStyle,
                radix: {
                    accentColor: radixAccentColor as NonNullable<NonNullable<PartnerConfig["theme"]["radix"]>["accentColor"]>,
                    grayColor: radixGrayColor as NonNullable<NonNullable<PartnerConfig["theme"]["radix"]>["grayColor"]>,
                    radius: radixRadius as NonNullable<NonNullable<PartnerConfig["theme"]["radix"]>["radius"]>,
                    scaling: radixScaling as NonNullable<NonNullable<PartnerConfig["theme"]["radix"]>["scaling"]>,
                    panelBackground: radixPanelBackground as NonNullable<NonNullable<PartnerConfig["theme"]["radix"]>["panelBackground"]>,
                    appearance: radixAppearance as NonNullable<NonNullable<PartnerConfig["theme"]["radix"]>["appearance"]>,
                },
            },
            ...
            footer: {
                ...
                poweredByLabel: footerPoweredByLabel.trim() || defaultFooterPoweredByLabel,
        ...
        const updatedConfig = updatePartnerFields(config, fields.filter((field) => enableReferralSection || field.sectionId !== "refer"));
        setSubmitting(true);
        setApiLoading(true);
        try {
    ...
    return (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={springTransition} className="rounded-lg border border-border bg-card p-6 form-container space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <details className="md:col-span-2 rounded-lg border border-border bg-muted/20 p-4">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <AdminSettingsFields vdn={vdn} brokerCode={brokerCode} vdnDirectoryEntries={vdnDirectoryEntries} vdnDirectoryLoading={isVdnDirectoryLoading} vdnDirectoryError={vdnDirectoryError} extraSettings={extraSettings} enableReport={enableFeedback} reportSearchFieldIds={reportSearchFieldIds} onVdnSelectionChange={(entry) => {
            setVdn(entry?.vdnNo ?? "");
            setBrokerCode(entry?.brokerCode ?? "");
        }} onExtraSettingsChange={setExtraSettings} onEnableReportChange={(enabled) => {
          ...
          <details className="md:col-span-2 rounded-lg border border-border bg-muted/20 p-4">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <AddFieldPanel sections={sections} availableFieldOptions={availableFieldOptions} groupedAvailableFieldOptions={groupedAvailableFieldOptions} selectedFieldId={selectedFieldId} newFieldSectionId={newFieldSectionId} newFieldWidth={newFieldWidth} newFieldRequired={newFieldRequired} onSelectedFieldIdChange={setSelectedFieldId} onNewFieldSectionIdChange={setNewFieldSectionId} onNewFieldWidthChange={setNewFieldWidth} onNewFieldRequiredChange={setNewFieldRequired} onAdd={addField}/>
            </div>
          </details>
          <details className="md:col-span-2 rounded-lg border border-border bg-muted/20 p-4">
            <summary className="cursor-pointer text-sm font-semibold text-foreground">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <AdminFooterFields footerLayout={footerLayout} footerShowPoweredBy={footerShowPoweredBy} footerPoweredByLabel={footerPoweredByLabel} footerPoweredByUrl={footerPoweredByUrl} footerLinksJson={footerLinksJson} footerLogosJson={footerLogosJson} footerNote={footerNote} onFooterLayoutChange={setFooterLayout} onFooterShowPoweredByChange={setFooterShowPoweredBy} onFooterPoweredByLabelChange={setFooterPoweredByLabel} onFooterPoweredByUrlChange={setFooterPoweredByUrl} onFooterLinksJsonChange={setFooterLinksJson} onFooterLogosJsonChange={setFooterLogosJson} onFooterNoteChange={setFooterNote}/>
            </div>
          </details>
        </div>

        <AdminConfiguredFieldsPanel groupedFields={groupedFields} sections={sections} onUpdateField={updateField} onRemoveField={removeField} onRemoveSection={removeSection}/>
