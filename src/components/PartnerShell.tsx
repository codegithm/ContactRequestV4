...
const PartnerShell = ({ config, children, variant = "form", }: PartnerShellProps) => {
    useEffect(() => {
    ...
    const maxWidth = variant === "report" ? "max-w-3xl" : "max-w-xl";
    ...
    const shellStyle = useMemo(() => ({
        ...
        "--partner-header-background": theme.headerBackground ||
            "var(--color-panel-translucent, rgba(255,255,255,0.92))",
        "--partner-footer-background": theme.footerBackground ||
            "var(--color-panel-translucent, rgba(255,255,255,0.9))",
        "--partner-card-background": theme.cardBackground ||
        ...
        "--partner-accent-shadow": theme.badgeStyle === "solid"
            ? `0 14px 28px hsl(${primary} / 0.2)`
            : `0 12px 24px hsl(${primary} / 0.12)`,
        "--partner-badge-background": theme.badgeStyle === "solid"
            ? `hsl(${primary})`
            : `hsl(${primary} / 0.86)`,
        "--partner-badge-shadow": theme.badgeStyle === "solid"
    ...
    return (<Theme appearance={radixTheme?.appearance ?? "light"} accentColor={radixTheme?.accentColor ?? "blue"} grayColor={radixTheme?.grayColor ?? "slate"} radius={radixTheme?.radius ?? "medium"} scaling={radixTheme?.scaling ?? "100%"} panelBackground={radixTheme?.panelBackground ?? "translucent"} asChild>
      <div className="min-h-svh flex flex-col partner-shell-bg" style={shellStyle}>
        ...
        <header className="w-full border-b border-border partner-header-surface">
          <div className={`${maxWidth} mx-auto px-6 py-4 flex items-center justify-between`}>
            <div className="flex items-center gap-3">
              {logoUrl ? (<img src={logoUrl} alt={`${config.partnerName} logo`} className="h-8 w-auto max-w-[180px] object-contain" loading="lazy"/>) : (<>
            ...
            <div className="flex items-center gap-3">
              {variant === "report" ? (<Button asChild variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground gap-1.5">
                  <Link href={`/${config.partnerId}`}>
                    <ArrowLeft className="w-3.5 h-3.5"/>
                    <span className="text-xs">Back to Form</span>
                  </Link>
                </Button>) : (<>
                  {config.feedback && (<Button asChild variant="outline" size="sm" className="gap-1.5 text-xs btn-press">
                      <Link href={`/${config.partnerId}/report`}>
        ...
        <main className="flex-1 flex items-start justify-center px-6 py-12">
          <div className={`w-full ${maxWidth}`}>
            {variant === "form" && (<div className="mb-8 space-y-2">
                <h1 className="text-2xl font-semibold tracking-tight" style={{ color: "var(--partner-headline-color)" }}>
                  {config.headline || `Contact ${config.partnerName}`}
                </h1>
                {config.description && (<p className="text-sm leading-relaxed" style={{ color: "var(--partner-description-color)" }}>
                    {config.description}
                  </p>)}
              </div>)}
            {variant === "form" ? (<div className="rounded-lg border overflow-hidden partner-card-surface">
                {showFormTopBanner && (<div className="border-b border-border/80 h-24 md:h-28 w-full bg-center bg-cover bg-no-repeat" style={{ backgroundImage: `url(${bannerUrl})` }} role="img" aria-label={`${config.partnerName} banner`}>
