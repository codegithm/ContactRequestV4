import { getAllPartnerIds, getPartnerConfig, type PartnerConfig, } from "@/lib/partners";
import { areApiCallsEnabled, buildApiUrl, getEnvironmentConfig, } from "@/config/environment";
...
function normalizePartnerConfig(config: PartnerConfig): PartnerConfig {
    ...
    if (normalizedSections.length === 0 && normalizedFields.length > 0) {
        ...
        if (unassigned.length > 0) {
            const mainSection = normalizedSections.find((section) => section.id === "main") ||
...
function mergePartners(base: PartnerConfig[], overrides: PartnerConfig[]): PartnerConfig[] {
...
class ApiPartnerConfigStore implements PartnerConfigStore {
    ...
    async upsert(config: PartnerConfig): Promise<void> {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error("Failed to save partner config");
        }
        catch {
            await this.fallback.upsert(config);
        }
...
export function createPartnerConfigStore(mode: "local" | "api" | "auto" = "auto"): PartnerConfigStore {
    if (mode === "local") {
