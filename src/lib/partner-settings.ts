...
export const extraPartnerSettingFieldIds = contactRequestModelFieldIds.filter(
  (fieldId): fieldId is ExtraPartnerSettingKey =>
    fieldId !== "vdn" && fieldId !== "brokerCode",
);
...
export const extraSettingFieldOptions: ExtraSettingFieldOption[] =
  extraPartnerSettingFieldIds.map((fieldId) => ({
    value: fieldId,
...
export function isExtraPartnerSettingKey(
  value: string,
): value is ExtraPartnerSettingKey {
  return extraPartnerSettingFieldIds.includes(value as ExtraPartnerSettingKey);
}

function inferDraftValueType(
  value: PartnerSettingValue,
): ExtraSettingValueType {
  if (value === null) {
    return "null";
  }
...
export function buildExtraSettingDrafts(
...
export function serializeExtraSettingDrafts(
  drafts: ExtraSettingDraft[],
): Partial<Record<ExtraPartnerSettingKey, PartnerSettingValue>> | undefined {
  const serializedEntries = drafts.reduce<
    Partial<Record<ExtraPartnerSettingKey, PartnerSettingValue>>
  >((result, draft) => {
    if (!draft.fieldId) {
      return result;
    }
...
export function getUnusedExtraSettingFieldOptions(
  drafts: ExtraSettingDraft[],
  currentDraftId?: string,
): ExtraSettingFieldOption[] {
  ...
  return extraSettingFieldOptions.filter(
    (option) => !usedFieldIds.has(option.value),
  );
}
